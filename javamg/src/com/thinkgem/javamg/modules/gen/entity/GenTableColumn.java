 ...
 * 
 */
package com.thinkgem.javamg.modules.gen.entity;
...
public class GenTableColumn extends DataEntity<GenTableColumn> {
	...
	public List<String> getAnnotationList() {
		...
		// 导入JSR303验证依赖包
		if (getIsNeedValidation() && !StringUtils.equals(getSimpleJavaField(), "id")) {
			if (!"1".equals(getIsNull())) {
				list.add("javax.validation.constraints.NotNull()");
			} 
			if ("String".equals(getJavaType()) && !"0".equals(getDataLength())) {
				if (getSimpleJavaField().toLowerCase().endsWith("id")) {
					list.add("org.hibernate.validator.constraints.Length(min=0, max=" + getDataLength() + ")");
				} else {
					list.add("org.hibernate.validator.constraints.Length(min=0, max=" + Integer.parseInt(getDataLength())/2 + ")");
				}
			} else if ("Integer".equals(getJavaType()) && !"0".equals(getDataLength())) {
				String max = "";
				for (int i = 0; i < Integer.parseInt(getDataLength()); i++) {
					if (i < 8) {
						max = max + "9";
					}
				}
				list.add("org.hibernate.validator.constraints.Range(min=0, max=" + max + ")");
			}
		}
		return list;
...
//		return !StringUtils.equals(getSimpleJavaField(), "createuserid")
//				&& !StringUtils.equals(getSimpleJavaField(), "createtime")
//				&& !StringUtils.equals(getSimpleJavaField(), "updateuserid")
//				&& !StringUtils.equals(getSimpleJavaField(), "updatetime")
//				&& !StringUtils.equals(getSimpleJavaField(), "isactive")
//				&& !StringUtils.equals(getSimpleJavaField(), "isdel");
		return true;
	}
	 ...
	 * @return
	 */
	public Boolean getIsNeedValidation() {
		return !StringUtils.equals(getSimpleJavaField(), "createuserid")
				&& !StringUtils.equals(getSimpleJavaField(), "createtime")
				&& !StringUtils.equals(getSimpleJavaField(), "updateuserid")
				&& !StringUtils.equals(getSimpleJavaField(), "updatetime")
